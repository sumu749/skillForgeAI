...
export async function getAllCourses(): Promise<Course[]> {
  if (getStorageMode() === "mongo") {
    ...
    return docs.map((d) => ({ ...(d as Record<string, unknown>), id: (d as { id?: string }).id || String((d as { _id?: unknown })._id) })) as Course[];
  }
...
export async function queryCourses(query: CourseQuery) {
  ...
  if (query.minPrice !== undefined) courses = courses.filter((c) => c.price >= query.minPrice!);
  if (query.maxPrice !== undefined) courses = courses.filter((c) => c.price <= query.maxPrice!);
  if (query.minRating !== undefined) courses = courses.filter((c) => c.rating >= query.minRating!);
...
export async function getReviewsByCourseId(courseId: string): Promise<Review[]> {
  if (getStorageMode() === "mongo") {
    const docs = await ReviewModel.find({ courseId }).sort({ createdAt: -1 }).lean();
    return docs.map((d) => ({
      id: d.id,
      courseId: d.courseId,
      userId: d.userId,
...
export async function seedCourses(courses: Course[], reviews: Review[]) {
  if (getStorageMode() === "mongo") {
    ...
    if (count === 0) {
      ...
      console.log(`✅ Seeded ${courses.length} courses and ${reviews.length} reviews to MongoDB`);
    }
    return;
  }
  writeJsonFile(COURSES_FILE, courses);
  writeJsonFile(REVIEWS_FILE, reviews);
  console.log(`✅ Seeded ${courses.length} courses and ${reviews.length} reviews to JSON storage`);
}

export async function getDashboardStats() {
  const courses = await getAllCourses();
  const reviews = getStorageMode() === "mongo"
  ...
  return {
    ...
    avgRating: courses.reduce((s, c) => s + c.rating, 0) / courses.length || 0,
    totalStudents: 2847,
    categoryCounts,
    monthlyEnrollments,
    topCourses: courses.sort((a, b) => b.reviewCount - a.reviewCount).slice(0, 5),
  };
}
